...
from save_profiletools_data import get_ne_Te
import quickfit_db
import matplotlib.pyplot as plt
from IPython import embed

sys.path.remove('/home/sciortino/tools3/neutrals')
import asp_probes, lyman_data
import aurora
...
def fit_2D_profs(shot,tmin=None,tmax=None,ne_zero_edge = False, Te_zero_edge = True,
    ...
    # get ne,Te data
    p_ne, p_Te = get_ne_Te(shot, [t_min,t_max], ntimes=None,  #ntimes=None--> no GPC downsampling
    ...
    try:
        ...
        out = asp_probes.get_clean_asp_data(shot, (t_max+t_min)/2., plot=False)
        rhop_asp, ne_prof_asp, ne_unc_prof_asp, Te_prof_asp, Te_unc_prof_asp, ax = out
        
        # ASP data is returned in SI units
        ne_prof_asp /= 1e20
        ne_unc_prof_asp /= 1e20
        Te_prof_asp /= 1e3
        Te_unc_prof_asp /= 1e3
        
        # shift rhop_asp to be from the magnetic axis
        rhop_asp += 1.0

        # mask out data points outside of range rhop=[0.995,1.02], often unrealiable
        mask_asp = (rhop_asp>0.995)&(rhop_asp<1.02)
        
        geqdsk = lyman_data.get_geqdsk_cmod(shot,1e3*(t_max+t_min)/2.)  
        geqdsk = lyman_data.get_geqdsk_cmod(shot,1e3*(t_max+t_min)/2.) # twice to be sure...
        roa_asp = aurora.rad_coord_transform(rhop_asp, 'rhop','r/a', geqdsk)
        
        ASP_X = np.ones((len(roa_asp),2))
        ASP_X[:,0] = (t_max+t_min)/2. * np.ones_like(roa_asp)  # time-indpt, place all points at one time
        ASP_X[:,1] = roa_asp
        
        p_ne.add_data(ASP_X[mask_asp,:], ne_prof_asp[mask_asp], err_y=ne_unc_prof_asp[mask_asp])
        p_Te.add_data(ASP_X[mask_asp,:], Te_prof_asp[mask_asp], err_y=Te_unc_prof_asp[mask_asp])
...
def Teu_2pt_model(shot,tmin,tmax,ne,Te,r_vec):
    '''
...
def clean_profs(shot,p_ne, p_Te, min_ne_err, max_ne_err, min_Te_err, max_Te_err):
    ...
    This is a rather arbitrary (and maybe not fully robust) method which is built by identifying 
...
def get_quickfit(roa_ne,time_ne,y_ne, err_y_ne, P_ne, W_ne,dt_ne,
    ...
    MG_ne.PrepareCalculation(zero_edge=ne_zero_edge,
    ...
    MG_Te.PrepareCalculation(zero_edge=Te_zero_edge,
                             core_discontinuties=sawteeth,edge_discontinuties= elms,
