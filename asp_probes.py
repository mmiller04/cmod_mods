...
# python version of Brian Labombard's get_asp_data.pro
...
def get_asp_data(shot,time):

    ASP = '\EDGE::TOP.PROBES.ASP'

    node_string = ASP + ':PLUNGE'
    node = OMFITmdsValue(server='CMOD', shot=shot, treename='EDGE', TDI=node_string)
    plunge = node.data()
    plunge_tm = node.dim_of(0)
    node_string = ASP + '.G_1:Rho'
    node = OMFITmdsValue(server='CMOD', shot=shot, treename='EDGE', TDI=node_string)
    raw_rho = node.data()
    ...
    probe_type = []
    for ip in range(4):
        node_string = ASP+'.G_1'+pnodes[ip]+':NAME'
        ...
        node_string = ASP+'.G_1'+pnodes[ip]+':PROCESSED'
        ...
        node_string = ASP+'.G_1'+pnodes[ip]+':PROBE_TYPE'
    ...
    for ii,side in enumerate(['N','S','E','W']):
        ...
        ip = int(pnum[ii])
        if psel[ip] == 1:
            node_string = ASP + '.G_1' + pnodes[ip] + ':OFFSET'
            offset = OMFITmdsValue(server='CMOD',shot=shot,treename='edge',TDI=node_string).data()
            node_string = ASP + '.G_1' + pnodes[ip] + ':VALID_FAST:INDICES'
            no_indices = 0
            ...
            if len(indices) > 0:
                _out = get_fsp_data_included(ASP,pnodes,ip,shot,no_indices,indices)
        ...
        if psel[ip] == 2:
            node_string = ASP + '.G_1' + pnodes[ip] + ':OFFSET'
            offset = OMFITmdsValue(server='CMOD',shot=shot,treename='edge',TDI=node_string).data()
            work_Vf,work_tbase = get_asp_vf_data_include(ASP,pnodes,ip,shot)
            inrange1 = np.where(work_tbase >= rho_scan_tm[0])[0]
            inrange2 = np.where(work_tbase <= rho_scan_tm[-1])[0]
            inrange = np.intersect1d(inrange1,inrange2)
            count = len(inrange)
...
def get_fsp_data_included(ASP,pnodes,ip,shot,no_indices,indices):
    ...
    node_string = ASP + '.G_1' + pnodes[ip] + ':PRO_AREA'
    ...
    node_string = ASP + '.G_1' + pnodes[ip] + ':TE_FAST'
    ...
    node_string = ASP + '.G_1' + pnodes[ip] + ':VF_FAST'
...
def get_asp_vf_data_include(ASP,pnodes,ip,shot):
...
def get_clean_asp_data(shot, time, plot=False):
    '''Simple interface to MDS+ to see if there exist ASP probe data for a given shot.
    If so, returns data collected nearest to the requested time (?).
    '''

    asp_out = get_asp_data(shot, time)
